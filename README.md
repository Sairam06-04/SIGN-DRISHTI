...
RealTime_Sign_Recognition/ ├── app_pyside.py # MAIN GUI APPLICATION (PySide6 entry point) ├── .gitignore # Ensures virtual environment, cache, and large data are ignored │ ├── scripts/ # Stores utility and sequence execution files │ ├── 1_data_collection.py # Script to capture and normalize landmark data │ ├── 2_train_model.py # Script to train the Logistic Regression classifier │ └── 3_app.py # Console-based detection script (Non-GUI version) │ ├── data/ # Stores collected sign language features │ └── sign_language_data.csv # Collected landmark data (generated by 1_data_collection.py) │ └── model/ # Stores the trained ML model └── sign_language_model.p # The trained Logistic Regression classifier (generated by 2_train_model.py)
...
1.  **Clone the repository:**
    ```bash
    git clone [https://github.com/YOUR_USERNAME/RealTime-Sign-Recognition.git](https://github.com/YOUR_USERNAME/RealTime-Sign-Recognition.git)
